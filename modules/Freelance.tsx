...
import { Briefcase, DollarSign, Clock, CheckCircle, Plus, Trash2, Calendar, User, MoreVertical, LayoutGrid, CalendarDays } from 'lucide-react';
...
const Freelance: React.FC = () => {
  ...
  const handleAddProj = () => {
      ...
      setProjName(''); setClient(''); setValue(''); setDeadline(''); setShowAdd(false);
  };
        ...
        {viewMode === 'grid' && (
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {activeProjects.map(p => (
                        <div key={p.id} className="glass-card rounded-xl p-5 hover:border-accent/50 transition-all group relative">
                            ...
                            <div className="flex justify-between items-end border-t border-border pt-4">
                                <div>
                                    ...
                                    <p className="text-sm font-medium flex items-center gap-1">
                                        <Calendar size={14} className="text-accent" /> {p.deadline ? formatDate(p.deadline, 'short') : 'No Date'}
                                    </p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-gray-500 mb-1">Value</p>
                                    <p className="text-lg font-mono font-bold text-white">{formatCurrency(p.value || 0)}</p>
                                </div>
                            </div>
                        </div>
                    ))}
                        ...
                        <tbody className="text-sm">
                            {completedProjects.map(p => (
                                <tr key={p.id} className="border-b border-border/50 hover:bg-bg-tertiary/20 opacity-70 hover:opacity-100 transition-opacity">
                                    ...
                                    <td className="p-4 text-right font-mono text-green-500">{formatCurrency(p.value || 0)}</td>
