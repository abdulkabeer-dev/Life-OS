...
import { Wallet, TrendingUp, TrendingDown, Plus, Trash2, ArrowUpRight, ArrowDownRight, PieChart, BarChart3, Activity, Edit2, MoreVertical, X, Save } from 'lucide-react';
...
const Finance: React.FC = () => {
  ...
  const [editForm, setEditForm] = useState({ description: '', amount: '', type: 'expense' as 'income' | 'expense', category: 'other' });
            ...
            <div className="glass-card rounded-xl p-6 border border-accent/50">
                ...
                <div className="mt-4">
                     ...
                     <select 
                       ...
                       className="w-full p-3 rounded-lg bg-bg-tertiary border border-border focus:border-accent outline-none capitalize"
                     >
                        {Object.keys(CATEGORY_COLORS).map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                </div>
                <div className="flex justify-end gap-3 mt-4">
                     ...
                     <button 
                       onClick={() => {
                         ...
                         if (txn) {
                           // Note: You may need to add updateTransaction function to context
                                                  ...
                                                  {menuOpen === t.id && (
                                                    <div className="absolute right-0 mt-1 w-32 bg-gray-900 border border-gray-700 rounded-lg shadow-lg z-20">
                                                      <button
                                                        onClick={() => {
                                                          setEditingId(t.id);
                                                          setEditForm({ description: t.description, amount: String(t.amount), type: t.type, category: t.category });
                                                          setMenuOpen(null);
                                                        }}
