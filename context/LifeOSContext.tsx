                  ...
                  doc(db, "users", currentUser.uid), 
                  {
                      next: (docSnap) => {
                          if (docSnap.exists()) {
                              ...
                              // Deep Merge defaults with proper Firebase data
                              const merged = {
                                  ...DEFAULT_DATA,
                                  ...remoteData,
                                  ...
                                  islam: {
                                      ...
                                      prayerHistory: remoteData.islam?.prayerHistory || []
                                  }
                              };
                              
                              isRemoteUpdate.current = true;
                              setData(merged);
                              console.log("✅ Data synced from Firebase for user:", currentUser.uid);
                          } else {
                              // New user, save default data
                              saveDataToFirestore(DEFAULT_DATA, currentUser.uid);
                              setData(DEFAULT_DATA);
                              console.log("✅ New user created, default data saved");
                          }
                          setLoading(false);
                      },
                      error: (error) => {
                          console.error("⚠️ Firestore sync error:", error);
                          setLoading(false);
                      }
                  }
              );
